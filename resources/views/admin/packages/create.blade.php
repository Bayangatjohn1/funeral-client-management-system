...
@section('page_desc', 'Create a new service package with pricing, inclusions, freebies, and optional add-ons.')
@section('content')
<style>
...
@php
    ...
    $initialInclusions = is_array(old('inclusions')) ? old('inclusions') : \App\Models\Package::parseLegacyItems(old('inclusions',''));
    $initialFreebies   = is_array(old('freebies'))   ? old('freebies')   : \App\Models\Package::parseLegacyItems(old('freebies',''));
    $initialAddOns     = is_array(old('add_ons'))     ? old('add_ons')    : [];
@endphp
<form id="packageCreateForm" method="POST" action="{{ route('admin.packages.store') }}"
...
<section class="pkg-section">
    <div class="mb-4"><div class="pkg-section-title">Basic Package Details</div>
    <p class="pkg-section-help">Set the package identity and base amount.</p></div>
    <div class="grid gap-4 md:grid-cols-2">
        <div class="md:col-span-2">
            <label class="pkg-label">Package Name <span class="text-[#9E4B3F]">*</span></label>
            <input type="text" name="name" value="{{ old('name') }}" class="form-input" placeholder="e.g. First Class Package" required>
        ...
        <div>
            ...
            <input type="text" name="coffin_type" value="{{ old('coffin_type') }}" class="form-input" placeholder="e.g. Premium Coffin" required>
        ...
        <div class="md:col-span-2">
            ...
            <label class="inline-flex items-center gap-2 text-sm text-[#5F685F] font-semibold cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active','1') ? 'checked' : '' }} class="h-4 w-4 rounded border-[#C9C5BB] text-[#3E4A3D] focus:ring-[#3E4A3D]">
                Active package
            </label>
        </div>
    </div>
...
<section class="pkg-section">
    ...
    <div class="space-y-4">
        ...
        <label class="inline-flex items-center gap-2 text-sm text-[#5F685F] font-medium cursor-pointer">
            ...
            <input type="checkbox" name="promo_is_active" value="1" {{ old('promo_is_active') ? 'checked':'' }} class="h-4 w-4 rounded border-[#C9C5BB] text-[#3E4A3D] focus:ring-[#3E4A3D]">
...
<section class="pkg-section pkg-section-alt pkg-actions md:col-span-2">
    ...
    <button class="btn pkg-primary-button px-5"><i class="bi bi-save2"></i> Save Package</button>
</section>
</form>
<script>

(function () {
    var formId = document.currentScript ? null : null;
    var form = document.querySelector('#packageCreateForm, #packageEditForm');
    if (!form) return;
